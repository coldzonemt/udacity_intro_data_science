...
def lineplot_compare(hr_by_team_year_sf_la_csv):
    ...
    #I think order matters - I moved up geom_line and then it rendered, but I still haven't been able to 
    #render the title. 
    
    data = pandas.read_csv(hr_by_team_year_sf_la_csv)
    gg = ggplot(data, aes(x="yearID", y="HR", color='teamID'))
    gg+geom_point() + geom_line() + ggtitle('SF vs. LA HRs') + xlab('Years') + ylab('# Home Runs')  
